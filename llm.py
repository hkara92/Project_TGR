...
Handles LLM calls (GPT-5-mini, Qwen2.5-7B, or LM Studio) and embedding
generation (OpenAI or BAAI BGE). Models are cached after first load
and freed with unload_model().
"""
...
def call_llm(prompt, model="gpt", max_tokens=4096):
    ...
    if model == "qwen":
        ...
        if _qwen_model is None:
            ...
            print("[LLM] Model loaded (bfloat16).")
...
def get_embeddings(texts, model="text-embedding-3-large"):
    ...
    if model == "bge":
        ...
        return _bge_embedder.encode(texts, show_progress_bar=len(texts) > 10, batch_size=16).tolist()
